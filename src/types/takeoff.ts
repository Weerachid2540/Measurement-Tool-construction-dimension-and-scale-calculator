...
/** พื้นและฝ้าคิดจากผังห้อง กว้าง × ยาว */
const areaInputs: readonly TakeoffInputDef[] = [
  { key: 'width', label: 'ความกว้างห้อง', unit: 'ม.', defaultValue: 4, min: 0, step: 0.1 },
  { key: 'length', label: 'ความยาวห้อง', unit: 'ม.', defaultValue: 5, min: 0, step: 0.1 },
];

const planArea = (v: Record<string, number>): number => n(v, 'width') * n(v, 'length');

/**
 * ขนาดแผ่นวัสดุเป็นตัวแปร — จำนวนแผ่นคิดจากเรขาคณิตล้วน ไม่ได้อิงตัวเลขจากผู้ผลิต
 * เผื่อเศษให้ใช้ช่อง "เผื่อเสียหาย" ของแถวแทน จะได้ไม่ซ้อนกันสองที่
 */
const sheetSizeInputs = (widthCm: number, lengthCm: number): TakeoffInputDef[] => [
  {
    key: 'sheetW',
    label: 'ขนาดแผ่น — ด้านกว้าง',
    unit: 'ซม.',
  ...
  {
    key: 'sheetL',
    label: 'ขนาดแผ่น — ด้านยาว',
    unit: 'ซม.',
    defaultValue: lengthCm,
    min: 1,
    step: 1,
    excludeFromWorking: true,
  },
...
const sheetsPerSqm = (v: Record<string, number>): number => {
  const area = (n(v, 'sheetW') / 100) * (n(v, 'sheetL') / 100);
  return area > 0 ? 1 / area : 0;
};

/**
 * สีคิดจากอัตราครอบคลุมที่ระบุข้างกระป๋อง × จำนวนเที่ยวที่ทา
 * ทั้งสองค่าเป็นช่องกรอก เพราะต่างยี่ห้อต่างสเปกกันมาก
 */
const paintInputs: TakeoffInputDef[] = [
  {
    key: 'coats',
    label: 'จำนวนเที่ยวที่ทา',
    unit: 'เที่ยว',
    defaultValue: 2,
    min: 1,
    step: 1,
...
const paintGallonsPerSqm = (v: Record<string, number>): number => {
  const coverage = n(v, 'coverage');
  return coverage > 0 ? n(v, 'coats') / coverage : 0;
};
