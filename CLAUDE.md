# Cost Estimation and Quantity Takeoff Services — บริบทสำหรับผู้ช่วย AI

เครื่องมือถอดแบบประมาณราคาและวัดขนาดจากแบบก่อสร้าง ทำงานทั้งหมดในเบราว์เซอร์ ไม่มี backend
(ชื่อโฟลเดอร์ ชื่อ package และคีย์ที่เก็บข้อมูลยังเป็น `measurement-tool` ตามเดิม — เปลี่ยนแค่ชื่อที่ผู้ใช้เห็น)
...
## หน้าถอดแบบ (แท็บ "ถอดแบบ")

คนละระบบกับ BOQ — BOQ มาจากรูปที่วัดบนแบบ ส่วนถอดแบบผู้ใช้กรอกตัวเลขเองล้วน ส่งออกคนละไฟล์

- `src/types/takeoff.ts` — ต้นทางเดียวของหมวดงาน: 4 หมวดหลัก → หมวดงาน → รายการงาน + **สูตร**
  เพิ่มรายการงานใหม่ที่นี่ที่เดียว UI กับไฟล์ส่งออกดึงไปเองทั้งหมด
- สูตรเก็บเป็นฟังก์ชัน `base(values)` + `openingFactor(values)` — งานที่ทำสองด้าน (ฉาบ/ผนังเบา)
  ต้องหักช่องเปิด × จำนวนด้าน ไม่ใช่หักครั้งเดียว
- ตอนนี้ใส่สูตรครบเฉพาะ **หมวดงานผนัง** (8 รายการ) หมวดอื่นมีแต่โครงรอเติม
- `TAKEOFF_GROUPS` ใช้ร่วมกับป้ายหมวดงานใน BOQ (`MaterialSpec.workGroup`) — ชื่อหมวดซ้ำข้ามหมวดหลักได้
  (เช่น "งานพื้น") จึงต้องใส่เลขหมวดนำหน้าเสมอเวลาเอาไปเป็น key
- ข้อมูลเก็บใน **localStorage** ไม่ใช่ IndexedDB เพราะไม่ผูกกับไฟล์แบบที่เปิดอยู่
